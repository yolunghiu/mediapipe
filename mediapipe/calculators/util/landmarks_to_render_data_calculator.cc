...
void SetColorSizeValueFromZ(float z,
                            float z_min,
                            float z_max,
                            ...
                            float max_depth_circle_thickness)
{
    ...
    const int thickness =
        static_cast<int>(min_depth_circle_thickness + (1.f - Remap(z, z_min, z_max, 1)) * scale);
                             ...
                             RenderData* render_data)
{
    for (int i = 0; i < landmark_connections.size(); i += 2)
    {
        ...
        if (!IsLandmarkVisibleAndPresent<LandmarkType>(
                ld0, utilize_visibility, visibility_threshold, utilize_presence, presence_threshold) ||
            !IsLandmarkVisibleAndPresent<LandmarkType>(
                ld1, utilize_visibility, visibility_threshold, utilize_presence, presence_threshold))
        {
            continue;
        }
        const Color color0 =
            MixColors(min_depth_line_color, max_depth_line_color, Remap(ld0.z(), min_z, max_z, 1.f));
        const Color color1 =
            MixColors(min_depth_line_color, max_depth_line_color, Remap(ld1.z(), min_z, max_z, 1.f));
        AddConnectionToRenderData<LandmarkType>(
            ld0, ld1, color0, color1, thickness, normalized, render_data);
    }
}
                    ...
                    RenderData* render_data)
{
    for (int i = 0; i < landmark_connections.size(); i += 2)
    {
        ...
        AddConnectionToRenderData<LandmarkType>(
            ld0, ld1, connection_color, thickness, normalized, render_data);
    }
}
...
absl::Status LandmarksToRenderDataCalculator::Process(CalculatorContext* cc)
{
    ...
    if (cc->Inputs().HasTag(kLandmarksTag))
    {
        ...
        if (visualize_depth)
        {
            AddConnectionsWithDepth<LandmarkList, Landmark>(landmarks,
                                                            landmark_connections_,
                                                            options_.utilize_visibility(),
                                                            options_.visibility_threshold(),
                                                            options_.utilize_presence(),
                                                            options_.presence_threshold(),
                                                            thickness,
                                                            /*normalized=*/false,
                                                            z_min,
                                                            z_max,
                                                            min_depth_line_color,
                                                            max_depth_line_color,
                                                            render_data.get());
        }
        else
        {
            AddConnections<LandmarkList, Landmark>(landmarks,
                                                   landmark_connections_,
                                                   options_.utilize_visibility(),
                                                   options_.visibility_threshold(),
                                                   options_.utilize_presence(),
                                                   options_.presence_threshold(),
                                                   options_.connection_color(),
                                                   thickness,
                                                   /*normalized=*/false,
                                                   render_data.get());
        }
        for (int i = 0; i < landmarks.landmark_size(); ++i)
        {
            ...
            if (!IsLandmarkVisibleAndPresent<Landmark>(landmark,
                                                       options_.utilize_visibility(),
                                                       options_.visibility_threshold(),
                                                       options_.utilize_presence(),
                                                       options_.presence_threshold()))
            ...
            auto* landmark_data_render =
                AddPointRenderData(options_.landmark_color(), thickness, render_data.get());
            if (visualize_depth)
            {
                SetColorSizeValueFromZ(landmark.z(),
                                       z_min,
                                       z_max,
    ...
    if (cc->Inputs().HasTag(kNormLandmarksTag))
    {
        ...
        {
            AddConnections<NormalizedLandmarkList, NormalizedLandmark>(landmarks,
                                                                       landmark_connections_,
                                                                       options_.utilize_visibility(),
                                                                       options_.visibility_threshold(),
                                                                       options_.utilize_presence(),
        ...
        for (int i = 0; i < landmarks.landmark_size(); ++i)
        {
            ...
            if (!IsLandmarkVisibleAndPresent<NormalizedLandmark>(landmark,
                                                                 options_.utilize_visibility(),
                                                                 options_.visibility_threshold(),
                                                                 options_.utilize_presence(),
                                                                 options_.presence_threshold()))
