...
absl::Status AnnotationOverlayCalculator::Process(CalculatorContext* cc)
{
    ...
    if (use_gpu_)
    {
        ...
        if (!gpu_initialized_)
        {
            MP_RETURN_IF_ERROR(gpu_helper_.RunInGlContext(
                [this, cc]() -> absl::Status { return GlSetup<mediapipe::GpuBuffer, kGpuBufferTag>(cc); }));
    ...
    if (use_gpu_)
    {
        ...
        MP_RETURN_IF_ERROR(gpu_helper_.RunInGlContext([this, cc, image_mat_ptr]() -> absl::Status {
...
absl::Status AnnotationOverlayCalculator::RenderToCpu(CalculatorContext* cc,
                                                      const ImageFormat::Format& target_format,
                                                      uchar* data_image)
{
    ...
    output_frame->CopyPixelData(target_format,
                                renderer_->GetImageWidth(),
                                renderer_->GetImageHeight(),
                                data_image,
...
absl::Status AnnotationOverlayCalculator::RenderToGpu(CalculatorContext* cc, uchar* overlay_image)
{
    ...
    auto output_texture =
        gpu_helper_.CreateDestinationTexture(width_, height_, mediapipe::GpuBufferFormat::kBGRA32);

    // Upload render target to GPU.
    {
        glBindTexture(GL_TEXTURE_2D, image_mat_tex_);
        glTexSubImage2D(
            GL_TEXTURE_2D, 0, 0, 0, width_canvas_, height_canvas_, GL_RGB, GL_UNSIGNED_BYTE, overlay_image);
...
absl::Status AnnotationOverlayCalculator::CreateRenderTargetCpu(CalculatorContext* cc,
                                                                std::unique_ptr<cv::Mat>& image_mat,
                                                                ImageFormat::Format* target_format)
{
    ...
    else
    {
        image_mat = absl::make_unique<cv::Mat>(
            options_.canvas_height_px(),
            options_.canvas_width_px(),
            CV_8UC3,
            cv::Scalar(
                options_.canvas_color().r(), options_.canvas_color().g(), options_.canvas_color().b()));
        *target_format = ImageFormat::SRGB;
    }
...
template <typename Type, const char* Tag>
absl::Status AnnotationOverlayCalculator::CreateRenderTargetGpu(CalculatorContext* cc,
                                                                std::unique_ptr<cv::Mat>& image_mat)
{
#if !MEDIAPIPE_DISABLE_GPU
    if (image_frame_available_)
...
absl::Status AnnotationOverlayCalculator::GlSetup(CalculatorContext* cc)
{
  ...
  DEFAULT_PRECISION(mediump, float)
    ...
    const std::string frag_src =
        absl::StrCat(mediapipe::kMediaPipeFragmentShaderPreamble, defines, kFragSrcBody);

    // Create shader program and set parameters
    mediapipe::GlhCreateProgram(mediapipe::kBasicVertexShader,
                                frag_src.c_str(),
                                NUM_ATTRIBUTES,
                                (const GLchar**)&attr_name[0],
                                attr_location,
                                &program_);
    ...
    // Init texture for opencv rendered frame.
    {
        ...
        glTexImage2D(
            GL_TEXTURE_2D, 0, GL_RGB8, width_canvas_, height_canvas_, 0, GL_RGB, GL_UNSIGNED_BYTE, nullptr);
