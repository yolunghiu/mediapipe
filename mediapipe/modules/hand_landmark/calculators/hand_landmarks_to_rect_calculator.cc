    ...
    NormalizedRect* rect)
{
    ...
    for (int i = 0; i < landmarks.landmark_size(); ++i)
    {
        const float original_x =
            (landmarks.landmark(i).x() - axis_aligned_center_x) *
            image_size.first;
...
public:
    ...
    absl::Status Process(CalculatorContext* cc) override
    {
        ...
        MP_RETURN_IF_ERROR(NormalizedLandmarkListToRect(
            landmarks, image_size, output_rect.get()));
        cc->Outputs()
            .Tag(kNormRectTag)
...
private:
    NormalizedLandmarkList GetPartialLandmarks(CalculatorContext* cc)
    {
        ...
        {
            static constexpr int kPartialLandmarkIndices[]{
        ...
        {
            // Assume the calculator is receiving the partial landmarks
            // directly. This is the legacy behavior.
            return landmarks;
        }
    }
};
