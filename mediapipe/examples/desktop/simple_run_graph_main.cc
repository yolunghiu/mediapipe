...
absl::Status RunMPPGraph()
{
        ...
        !absl::GetFlag(FLAGS_output_stream_file).empty())
    {
        ASSIGN_OR_RETURN(
            auto poller,
            graph.AddOutputStreamPoller(absl::GetFlag(FLAGS_output_stream)));
    ...
    else
    {
        RET_CHECK(absl::GetFlag(FLAGS_output_stream).empty() &&
                  absl::GetFlag(FLAGS_output_stream_file).empty())
            << "--output_stream and --output_stream_file should be specified "
               "in "
               "pair.";
        LOG(INFO) << "Start running the calculator graph.";
        MP_RETURN_IF_ERROR(graph.StartRun({}));
