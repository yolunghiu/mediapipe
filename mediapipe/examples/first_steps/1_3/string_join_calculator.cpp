...
public:
    ...
    {
        using namespace std;
        // When calculator has more than one input or output, anonymous streams
        // become messy Here we demonstrate the use of tags, and also tag+number
        // syntax 2 inputs: "STR:0:" and "STR:1:", and one output "STR:" of type
        // sdt::string
    ...
    Status Process(CalculatorContext* cc) override
    {
        ...
        cout << "At timestamp **" << cc->InputTimestamp().Value() << "**";
        ...
        // If, say, timestamp 12 is missing in one stream, the packet will be
        // empty At least one input packet is always nonempty
        string s1("<EMPTY>"), s2("<EMPTY>");
        if (!pIn1.IsEmpty()) s1 = pIn1.Get<string>();
        if (!pIn2.IsEmpty()) s2 = pIn2.Get<string>();
