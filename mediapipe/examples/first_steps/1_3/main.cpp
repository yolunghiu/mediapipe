...
mediapipe::Status run()
{
    ...
    // Here I introduce a "time shift" tTshift for "in" timestamps, always 15
    // packets But timestamps of stream "gen" (generated by
    // StringSourceCalculator) Always start with 0, and there are 17 packets How
    // does MP sync two streams? Try it and find out!
    int tShift = -2;
    for (int i = 0; i < 15; ++i)
    {
        mediapipe::Timestamp ts(i + tShift);
        mediapipe::Packet packet =
            mediapipe::MakePacket<string>("BRIANNA" + to_string(i)).At(ts);
