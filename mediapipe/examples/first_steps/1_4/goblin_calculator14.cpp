...
public:
    static Status GetContract(CalculatorContract* cc)
    {
        ...
        cout << "GoblinCalculator14::GetContract()" << endl;
    ...
    {
        using namespace std;
        // Copy options to class fields (a good style, but in principle we can
        // access them directly in Process)
        auto options = cc->Options<GoblinCalculator14Options>();
        a = options.opt_a();
        b = options.opt_b();
        cout << "GoblinCalculator14::Open() : a = " << a << ", b = " << b
             << endl;
        return OkStatus();
    }
