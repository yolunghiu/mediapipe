...
/// We define a data class GoblinCalculator14
/// Options which is a protobuff class
...
mediapipe::Status run()
{
    ...
    // Standard MP calculators use options a lot, get used to this somewhat
    // bizarre syntax! Try to modify the two options!
    string protoG = R"(
    input_stream: "in"
    output_stream: "out"
    node {
    ...
    if (!mediapipe::ParseTextProto<mediapipe::CalculatorGraphConfig>(protoG,
                                                                     &config))
    {
