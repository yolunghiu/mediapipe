...
/// Running all mediapipe stuff inside a functions which returns
/// mediapipe::Status is convenient, as you can use MP macroses like
/// MP_RETURN_IF_ERROR mediapipe::Status is absl::Status from abseil under the
/// hood
mediapipe::Status run()
{
    ...
    // Note that ParseTextProtoOrDie crashes program on error, see better option
    ...
    auto cb = [](const mediapipe::Packet& packet) -> mediapipe::Status {
        ...
        cout << "ISEMPTY = " << packet.IsEmpty()
             << ", VALIDATE = " << packet.ValidateAsType<string>() << endl;

        // We also print the timestamp
        cout << "out CallBack" << packet.Timestamp() << ": RECEIVED PACKET "
             << packet.Get<string>() << endl;

        // It is vital to always return OK status, unless you want to stop the
        // graph execution
    ...
    // Atatch this callback as observer to the graph output stream out1
    /*auto cb1 = [](const mediapipe::Packet& packet) -> mediapipe::Status {
        cout << "out1 CallBack" << packet.Timestamp() << ": RECEIVED PACKET "
             << packet.Get<string>() << endl;

        return mediapipe::OkStatus();
    };
    MP_RETURN_IF_ERROR(graph.ObserveOutputStream("out1", cb1));*/

    /// Run the graph, it usually starts in parallel threads and waits for input
    /// data
    ...
    // Close the input stream: signal MP that no more packets will be sent to
    // "in"
    ...
    return mediapipe::OkStatus();
    // These two lines can be replaced by (in simplest cases only, thus I avoid
    // this syntax) return graph.WaitUntilDone();
}
