...
/// In the previous example, we saw how a slow calculator breaks down a
/// real-time pipeline. We need to lose some packets to prevent queue (buffer)
/// overflow. This is a standard situation in real-time pipelines. Solution?
/// FlowLimiterCalculator ! It starts to lose packets if the buffer AFTER
/// FlowLimiterCalculator fills up. Result: The output video has low FPS, but no
/// ever-increasing lag, and does not fill all RAM
...
mediapipe::Status run()
{
    ...
    // Note how FlowLimiterCalculator receives the output stream out, the final
    // output as a second input stream. A loopback! It is used so that
    // FlowLimiterCalculator can see the current buffer size by comparing the
    // timestamps of two input streams
    string protoG = R"(
        input_stream: "in"
        output_stream: "out"
        node {
    ...
    for (int i = 0; !flagStop; ++i)
    {
        ...
        Timestamp ts(i);
        MP_RETURN_IF_ERROR(
            graph.AddPacketToInputStream("in", Adopt(inputFrame).At(ts)));
    }
