...
/// Here we use OpenCV, and MP has its own wrappers for opencv headers (How
/// stupid is this?) MP uses an mutable class ImageFrame for images One must
/// convert cv::Mat <-> ImageFrame back and forth Note that cv::Mat uses BGR by
/// default, while ImageFrame uses RGB by default We stick to this convention
/// and put cv::cvtColor when needed
...
mediapipe::Status run()
{
    ...
    auto cb = [](const Packet& packet) -> Status {
        ...
        // Represent ImageFrame data as cv::Mat (MatView is a thin wrapper, no
        // copying)
    ...
    for (int i = 0;; ++i)
    {
        ...
        // Create an empty (black?) RGB ImageFrame with the same size as our
        // image. Note the raw pointer. Is there a memory leak? NO, because of
        // Adopt(), see below
        ...
        uint64 ts = i;
        // Adopt() creates a new packet from a raw pointer, and takes this
        // pointer under MP management. So that you must not call delete on the
        // pointer after that, MP will delete your object automatically when the
        // packet is destroyed. This is like creating shared_ptr from a raw
        // pointer. This is useful for the classes which cannot be (easily)
        // copied, like ImageFrame. Note that MakePacket<...>() we used
        // previously contains a move or copy operation
