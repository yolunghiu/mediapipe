...
public:
    ...
    {
        absl::SleepFor(
            absl::Milliseconds(300));  // For GetOutputSidePacket test.
        cc->OutputSidePackets().Index(0).Set(
        ...
        PacketTypeSet* outputs)
    {
        if (!inputs->TagMap()->SameAs(*outputs->TagMap()))
        {
            return absl::InvalidArgumentError(
                "Input and outputs to PassThroughGenerator must use the same "
                "tags "
                "and indexes.");
        }
        for (CollectionItemId id = inputs->BeginId(); id < inputs->EndId();
             ++id)
        {
...
TEST(CalculatorGraph, SharePacketGeneratorGraph)
{
    ...
    MP_ASSERT_OK(
        generator_graph.Initialize(&validated_generator_config,
                                   nullptr,
...
TEST(CalculatorGraph, SourceLayerInversion)
{
    ...
    MP_ASSERT_OK(graph.Initialize(
        config,
        {{"max_count", MakePacket<int>(max_count)},
...
TEST(CalculatorGraph, GetOutputSidePacketAfterCalculatorIsOpened)
{
    ...
    MP_ASSERT_OK(graph.WaitUntilIdle());
    absl::StatusOr<Packet> status_or_packet =
        graph.GetOutputSidePacket("offset");
    MP_ASSERT_OK(status_or_packet);
