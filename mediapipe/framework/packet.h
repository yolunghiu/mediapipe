...
std::shared_ptr<const T> SharedPtrWithPacket(Packet packet)
{
    ...
    return std::shared_ptr<const T>(
        ptr,
        [packet = std::move(packet)](const T* ptr) mutable { packet = {}; });
}
...
public:
    ...
    // underlying object is protocol buffer type, otherwise, nullptr is
    // returned.
...
struct MessageRegistrationImpl
{
    ...
    // This could have been a lambda inside registration's initializer below,
    // but MSVC has a bug with lambdas, so we put it here as a workaround.
...
NoDestructor<mediapipe::RegistrationToken>
    MessageRegistrationImpl<T>::registration(MessageHolderRegistry::Register(
        T{}.GetTypeName(),
        MessageRegistrationImpl<T>::CreateMessageHolder));
...
template <typename T>
struct HolderSupport<T, typename std::enable_if<is_concrete_proto_t<T>{}>::type>
{
    // We must use std::remove_cv to ensure we don't try to register Foo twice
    // if there are Holder<Foo> and Holder<const Foo>. TODO: lift this up to
    // Holder?
    ...
    // referenced in a context that requires the definition to exist (see
    // ISO/IEC C++ 2003 standard, 14.7.1). Calling this ensures that's the case.
    ...
    static void EnsureStaticInit()
    {
        CHECK(R::registration.get() != nullptr);
    }
};
...
protected:
    ...
    // Returns the MessageLite pointer to the data, if the underlying object
    // type is protocol buffer, otherwise, nullptr is returned.
    ...
    {
        return ConvertToVectorOfProtoMessageLitePtrs(ptr_,
                                                     is_proto_vector<T>());
    }
...
private:
        ...
        typename std::enable_if<std::is_array<U>::value>::type* = 0)
    {
        ...
        // Deleting a pointer to incomplete type (Type(*)[]) causes compile
        // error.
                            ...
                            std::extent<T>::value != 0>::type*)
{
    ...
    std::copy(std::begin(original_array),
